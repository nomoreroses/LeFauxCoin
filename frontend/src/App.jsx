...
const ScamScanner = () => {
  ...
  const handleDescriptionChange = (e) => {
    ...
    if (memberMatch && memberMatch[1]) { newData.accountYear = parseInt(memberMatch[1]); newDetected.member = true; }
                    ...
                    <div className="space-y-3">
                        {uniqueDetails.map((item, i) => (
                            <div key={i} className="flex gap-4 p-4 rounded-2xl bg-orange-50 border border-orange-100 items-start hover:shadow-md transition">
                                ...
                                <div><h4 className="font-bold text-slate-800 text-sm">{item.label}</h4><p className="text-sm text-slate-600 mt-1 leading-relaxed">{item.desc}</p></div>
                            </div>
                        ))}
                    </div>
                </section>
